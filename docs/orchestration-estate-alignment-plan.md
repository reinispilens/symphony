...
The following work exists only as local, uncommitted candidate changes on 2026-08-25. It is useful
implementation evidence, but it is not yet merged authority and does not by itself satisfy a phase
exit:

| Phase | Local candidate evidence                                                                                                                                                                                                                                                                                                                                                                                                                            | Still required before exit                                                                                                      |
| ----- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| 0     | Reviewed `GP-01`–`GP-20` candidate, migration freeze/routing edits, and a green 35-test publication contract exist in the personal `.github` checkout                                                                                                                                                                                                                                                                                               | Owner acceptance through that repository; portable publication reference                                                        |
| 1     | WorkSession document v2 now includes pinned profile/context/binding, plans/decisions, session-level human attachment, attempts, materialization/proof/delivery, v1→v2 migration, outbox, backup, and green `pnpm check`/`pnpm build` (29 files, 185 tests)                                                                                                                                                                                          | Commit/merge the reviewed candidate; null tracker configuration/doctrine remain explicit compatibility states until Phases 2/6b |
| 2     | RepositoryDriver; guarded Git worktrees; exact-revision product profile/operator binding split; exact scrubbed Git with hook/filter refusal; separate state root; refusal of positional managed-Git authority; exact Codex sandbox/systemd scope; quiescence-before-expiry; accepted-binding daemon fixture; combined green `pnpm check`/`pnpm build`                                                                                               | Review and commit/merge; merged authority is not claimed locally                                                                |
| 3     | Offline-only pnpm v2 record; exact profile/binding class match with no dormant pnpm authority for `none`; strict manifest/workspace/lock source + SHA-512 admission; exact operator toolchain/policy; read-only seed with Attempt-private index/cache; no network namespace; timeout/cancellation descendant teardown; real dependency and hostile probes; green `pnpm check`/`pnpm build` (227 passed, 1 host skip) plus enabled 1-test host proof | Review and commit/merge; local green evidence is not accepted authority                                                         |
| 4–9   | No phase exit claimed                                                                                                                                                                                                                                                                                                                                                                                                                               | Implement and prove each owning-repository phase in the order below                                                             |

Separately, WCP `main` advanced during this review to `3628446`, with an accepted enabled max-one
Dyslexify GitHub/GARM declaration whose connected live proof is still pending. That is current
infrastructure baseline—not a local Symphony change and not Phase 4 proof-v2 completion. The plan
below reuses it and removes the previously proposed direct Symphony transport.
